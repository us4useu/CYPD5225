        ...
        void)
{
    ...
    mode   = 0x91;              /* Dual boot, 256 byte flash, 1 port, FW1 running. */
...
static volatile bool g_vsys_removed = false;
...
{
    /* Set a flag that triggers port disable operation. */
    g_vsys_removed = true;
}

static void port_disable_cb(uint8_t port, dpm_typec_cmd_resp_t resp)
{
    (void)port;
    (void)resp;
}
...
int main()
{
    ...
    /* Init NCP81239 Power Delivery Controller */
    pd_ctrl_init();
    ...
    while(1)
    {
        ...
        {
            /* 
               VSYS removed and we are not supposed to operate in dead battery mode.
               Disable all PD ports (this should cause a reset automatically).
             */
            g_vsys_removed = false;
            for (i = TYPEC_PORT_0_IDX; i < NO_OF_TYPEC_PORTS; i++)
            {
                dpm_typec_command (i, DPM_CMD_PORT_DISABLE, port_disable_cb);
            }
        }
#endif /* CCG5_DEADBAT_DISABLE */
    }
