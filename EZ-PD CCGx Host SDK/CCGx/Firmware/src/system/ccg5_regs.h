 ...
 *
 * Version: M0S8-Product-CCG5.xlsx
 *
 * Description:
...
/*
 * When Rp is connected: FW should set CC1_ATTACH_VALUE and CC2_ATTACH_VALUE
 * register bits to value that will be seen if Rd is attached by the partner.
 * HW will compare these programmed register values to the output of the
 * CC1/CC2 attach comparator to declare attach. The filter will be reset
 * by HW to 0 before enabling Rp.
 */
...
/*
 * When Rd is connected: Two options are provided.
 * 1. VBUS detect. This can be overridden by setting VBUS_DET_OVERRIDE
 * 2. Rp detect on CC: This can be overridden by setting VCMP_CC_OVERRIDE.
 * The comparator and filter required for this detection need to be configured
 * before DRP is enabled. Hardware will reset the filter to 1 before enabled
 * Rd.
 */
...
/*
 * This field specifies the reset value of the Filter. To reset the Filter,
 * set this bit to the appropriate value and Toggle FILTER_EN. Depending
 * on the clock on which the Filter is operating it will take upto 5 cycles
 * for the Filter to be operational again. So FILTER_EN should not be toggled
...
/*
 * Selection for SCP  comparator. This selection ranges from 200mV to 2190mV
 * for 0 to 199 respectively. With the extra bits meaning a floating (unconnected)
 * output
 */
...
/*
 * Selection for OCP comparator. This selection ranges from 200mV to 2190mV
 * for 0 to 199 respectively. With the extra bits meaning a floating (unconnected)
 * output
 */
...
/*
 * Selection for PFC_disable comparator. This selection ranges from 200mV
 * to 2190mV for 0 to 199 respectively. With the extra bits meaning a floating
 * (unconnected) output
 */
...
/*
 * Selection for PFC_enable comparator. This selection ranges from 200mV
 * to 2190mV for 0 to 199 respectively. With the extra bits meaning a floating
 * (unconnected) output
 */
...
/*
 * Selection for SR_disable comparator. This selection ranges from 200mV
 * to 2190mV for 0 to 199 respectively. With the extra bits meaning a floating
 * (unconnected) output
 */
...
/*
 * Selection for SR_enable comparator. This selection ranges from 200mV to
 * 2190mV for 0 to 199 respectively. With the extra bits meaning a floating
 * (unconnected) output
 */
...
/*
 * Selection for EA. This selection ranges from 200mV to 2190mV for 0 to
...
/*
 * Selection for ADC_Ref. This selection ranges from 1960mV to 2040mV for
...
/*
 * Selection for CC_SHVT Ref. This selection ranges from 1080mV to 1160mV
 ...
 * t_vconn[0]=1 Disable the reference generation of ovp
 * t_vconn[1]=1 Disable trim controlled ovp detection circuit
 * t_vconn[2]=1 Disable crude ovp detection(supply+vtp) circuit
 * t_vconn[3]=1 Increase self bias geneation stack current
 * t_vconn[4]=1 Decrease self bias geneation stack current
 * t_vconn[5]=1 Enable extra current sink to discharge the charge pump output
 * during ovp detects
 * t_vconn[6]=1 Increase current sink multiplier to discharge the charge
 * pump ouput durung ovp detects
 * t_vconn[7]=1 Decrease current sink multiplier to discharge the charge
 * pump ouput durung ovp detects
 * t_vconn[8]=1 Enable the backup clock for the charge pump
 ...
 * t_vconn[11]=1 Increase the comparator slew rate of trim controlled ovp
 * detects
 * t_vconn[12]=1 Disable crude ocp detection circuit
 * t_vconn[13]=1 Disable auto shut down of vconn when ovp is detected
 * t_vconn[14]=1 Connect v5v spupply to vddd supply, when v5v is not present
 * to prevent latch-up
 * t_vconn[15]  is reserved
 */
 ...
 * When SBU20_ADFT[5:0] = 000001, then SBU adft[1] = Z and adft[0] = sbu1_int.
 * When SBU20_ADFT[5:0] = 000010, then SBU adft[1] = Z and adft[0] = sbu2_int.
 * When SBU20_ADFT[5:0] = 000011, then SBU adft[1] = Z and adft[0] = aux_p.
 * When SBU20_ADFT[5:0] = 000100, then SBU adft[1] = Z and adft[0] = aux_n.
 * When SBU20_ADFT[5:0] = 000101, then SBU adft[1] = Z and adft[0] = lstx.
 * When SBU20_ADFT[5:0] = 000110, then SBU adft[1] = Z and adft[0] = lsrx.
 * When SBU20_ADFT[5:0] = 000111, then SBU adft[1] = Z and adft[0] = sbu1.
 * When SBU20_ADFT[5:0] = 001000, then SBU adft[1] = Z and adft[0] = sbu2.
 * When SBU20_ADFT[5:0] = 001001, then SBU adft[1] = Z and adft[0] = Z.
 * When SBU20_ADFT[5:0] = 001010, then SBU adft[1] = ovp_sbu1 and adft[0]
 * = sbu1_isink0.
 * When SBU20_ADFT[5:0] = 001011, then SBU adft[1] = ovp_sbu1 and adft[0]
 * = sbu1_isink1.
 * When SBU20_ADFT[5:0] = 001100, then SBU adft[1] = ovp_sbu1 and adft[0]
 * = sbu1_isink2.
 * When SBU20_ADFT[5:0] = 001101, then SBU adft[1] = ovp_sbu2 and adft[0]
 * = sbu2_isink0.
 * When SBU20_ADFT[5:0] = 001110, then SBU adft[1] = Z and adft[0] = sbu2_isink1.
 * When SBU20_ADFT[5:0] = 001111, then SBU adft[1] = Z and adft[0] = sbu2_isink2.
 ...
 * t_sbu[7]=1 Decrease current sink multiplier to discharge the charge pump
 * ouput durung ovp detects
 ...
 * When DPDM_T_DPDM[3:0] = 0000, charger detect isolation switch is OFF.
 * When DPDM_T_DPDM[3:0] = 1000, charger detect isolation switch ON on pump domain.
 * When DPDM_T_DPDM[3:0] = 1100, charger detect isolation switch ON on vddio domain.
 */
